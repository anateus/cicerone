...
func (s *Store) ApplyHistory(ctx context.Context, batch HistoryBatch) error {
	return s.Write(ctx, func(tx *sql.Tx) error {
		if err := applyHistoryRows(ctx, tx, batch); err != nil {
			return err
		}
		return finalizeHistory(ctx, tx, batch)
	})
}

func (s *Store) ApplyHistoryBatch(ctx context.Context, batch HistoryBatch) error {
	return s.Write(ctx, func(tx *sql.Tx) error { return applyHistoryRows(ctx, tx, batch) })
}

func (s *Store) FinalizeHistory(ctx context.Context, batch HistoryBatch) error {
	return s.Write(ctx, func(tx *sql.Tx) error { return finalizeHistory(ctx, tx, batch) })
}
